...
class gate(nn.Module):
    def __init__(self, input_dim, output_dim, units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
        ...
        self.w = torch.empty(output_dim, input_dim, device=device)
        self.u = torch.nn.Parameter(torch.nn.init.uniform_(self.w, self.e, 1 - self.e),
                                    requires_grad=True)

        self.w_matrix = [[torch.empty(units, units, device=device) for j in range(self.input_dim)] for i in range(self.output_dim)]

        self.trans_matrix = [[torch.nn.Parameter(torch.nn.init.xavier_normal_(self.w_matrix[i][j]),
                                           requires_grad=True) for j in range(self.input_dim)] for i in range(self.output_dim)]
    ...
    def forward(self, x):
        ...
        for i in range(self.output_dim):
            outputs = torch.stack([torch.matmul(x[j], self.trans_matrix[i][j]) * self.z_params[i][j] for j in range(self.input_dim)], 1)
            outputs = outputs.sum(1)
            gate_outs.append(outputs)
        return gate_outs

class SNR_trans(BaseModel):
    # def __init__(self, config):
    def __init__(self, dnn_feature_columns, init_std=0.0001, device='cpu', gpus=None, config=None):
        super(SNR_trans, self).__init__(linear_feature_columns=[], dnn_feature_columns=dnn_feature_columns,
        ...
        self.trans = nn.ModuleDict({'trans1': nn.ModuleList([DNN(self.input_dim, [self.expert_dnn_hidden_units[0]], activation=dnn_activation,
        ...
        if len(self.expert_dnn_hidden_units) == 1:
            self.trans['gate1'] = gate(self.num_experts, self.num_tasks, self.expert_dnn_hidden_units[-1], activation=dnn_activation,
                                            l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                                            init_std=init_std, device=device)
        else:
            self.trans['gate1'] = gate(self.num_experts, self.num_experts, self.expert_dnn_hidden_units[0], activation=dnn_activation,
                                             l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                                             init_std=init_std, device=device)
            for i in range(1, len(self.expert_dnn_hidden_units)):
                self.trans['trans{}'.format(i + 1)] = nn.ModuleList([DNN(self.expert_dnn_hidden_units[i - 1], [self.expert_dnn_hidden_units[i]], activation=dnn_activation,
                                             l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                                             init_std=init_std, device=device) for _ in range(self.num_experts)])
                if i == len(self.expert_dnn_hidden_units) - 1:
                    self.trans['gate{}'.format(i + 1)] = gate(self.num_experts, self.num_tasks, self.expert_dnn_hidden_units[-1], activation=dnn_activation,
                                             l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                                             init_std=init_std, device=device)
                else:
                    self.trans['gate{}'. format(i + 1)] = gate(self.num_experts, self.num_experts, self.expert_dnn_hidden_units[i], activation=dnn_activation,
                                             l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn,
                                             init_std=init_std, device=device)
        ...
        for i in range(len(self.expert_dnn_hidden_units)):
            ...
            if i == 0:
                for j in range(self.num_experts):
                    expert_out = self.trans['trans{}'.format(i + 1)][j](dnn_input)
                    outs.append(expert_out)
                # expert_outs = outs

            else:
                for j in range(self.num_experts):
                    expert_out = self.trans['trans{}'.format(i + 1)][j](gate_outs[j])
                    outs.append(expert_out)
            gate_outs = self.trans['gate{}'.format(i + 1)](outs)
